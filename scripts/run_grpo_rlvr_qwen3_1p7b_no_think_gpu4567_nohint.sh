...
DATA_DIR="$ROOT/data/rlvr_nohint_onpolicy_grpo_v1"

python "$ROOT/scripts/build_grpo_rlvr_dataset.py" \
  --input-jsonl "$DATA_SRC" \
  --out-dir "$DATA_DIR"

cp "$DATA_DIR/val.parquet" "$DATA_DIR/train.parquet"

EXP="grpo_qwen3_1p7b_rlvr_nohint_no_think_is_4gpu_$(date +%Y%m%d_%H%M%S)"
...
export RAY_TMPDIR="/tmp/ray_shp_4567"
export TMPDIR="/tmp/tmp_shp_4567"
...
# tensorboard --logdir /home/seankopylov/projects/solve-hard-problems/runs/grpo_qwen3_1p7b_rlvr_nohint_no_think_is_4gpu_20260305_211235/tensorboard --host 0.0.0.0 --port 12621
# ssh -L 12621:localhost:12621 ***